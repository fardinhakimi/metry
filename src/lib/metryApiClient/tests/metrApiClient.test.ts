...
const meters: Metry.ApiMeterListResult = 
 ...
 describe('MetryApiClient', () => {
    ...
    test("It should return the correct consumption value for each meter", async () => {
      ...
      fetchMock.get(`${BASE_URL}/meters?box=active&metrics=energy`, meters)
               .get(`${BASE_URL}/consumptions/605db28dbb5f5735f53653f5/month/2021-202108?metrics=energy`, consumptionForFirstMeter)
               .get(`${BASE_URL}/consumptions/705db28dbb5f5735f53653d5/month/2021-202108?metrics=energy`, consumptionForSecondMeter)
      ...
      expect(items.filter(item => item._id === '705db28dbb5f5735f53653d5')[0].consumption?.value).toEqual(2021)
      expect(items.filter(item => item._id === '605db28dbb5f5735f53653f5')[0].consumption?.value).toEqual(2020)
      expect(items.filter(item => item._id === '775db28dbb5f5735f53653d5')[0].consumption).toEqual(null)
      fetchMock.mockReset()
    })
})
