...
export class MetryApiClient implements Metry.IMetriApiClient, Metry.IFilterAble {
    ...
    setBoxStatus(status: Metry.MeterBoxStatus = Metry.MeterBoxStatus.ACTIVE): MetryApiClient {
    ...
    private getLatestConsumption(consumption: Metry.Consumption) {
        ...
        const [ headPeriodItem ] = consumption.data[0].periods.filter( item => {
            return item.hasOwnProperty(metric)
        })
    ...
    async getMeters(): Promise<Metry.MeterListResult> {
        ...
        try {
            ...
            const results = await Promise.all( meterList.data.map( async (meter: Metry.Meter ): Promise<Metry.MeterListItem> => {
                ...
                try {
                    ...
                    if(revoked || ![Metry.MeterBoxStatus.ACTIVE, Metry.MeterBoxStatus.TEMPORARY].includes(box)) {
                        return defaultMeter
                    }
    ...
    async getConsumption(meterId: string): Promise<Metry.Consumption> {
        ...
        if(!meterId || !this.consumptionParams.period || !this.consumptionParams.granularity) {
            throw new Error('MeterId, format, period and granularity is required to get a meaninful consumption')
        }
        ...
        const endPeriod = period.end ?  dateformat(period.end.date, period.end.format) : ''
